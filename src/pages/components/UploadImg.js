...
class UploadImg extends Component {
    ...
    handleBeforeUpload = (file) =>
        new Promise((reject) => {
            ...
            // const isLt2M = file.size / 1024 / 1024 < defaultSize
            if (!isJPG) {
                message.error("请选择图片文件!")
                // if (!isLt2M) {
                //     message.error("单张图片大小不能超过2M!")
                // }
            }
    ...
    render () {
        const { imgList, previewVisible, previewImage, maxNum } = this.state
        const { divWidth } = this.props
        uploadProps = {
            ...uploadProps,
            multiple: true,
            fileList: imgList,
        ...
        return (
            <div id="imgBox" className={styles.uplodBox} onChange={this.props.uploadNum && this.props.uploadNum()}>
                <div className={styles.uplodDiv} style={{ width: divWidth || "100%", minWidth: divWidth || 200 }} title="可拖拽上传">
...
function mapStateToProps (state) {
    return {
        uptoken: state.material.uptoken,
    }
}
